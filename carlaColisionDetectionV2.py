...
try:
    ...
    def _on_collision(data):
        ...
        actor_type = get_actor_display_name(data.other_data)
        print("collision with", actor_type)
        collision_event_record = data.normal_impulse
        intensify_of_collision = math.sqrt(collision_event_record.x ** 2 + collision_event_record.y **2 + collision_event_record.z ** 2)
        print("intensify_of_collision", intensify_of_collision)
        dropped_vehicle.apply_control(carla.VehicleControl(hand_brake=True))
        time.sleep(5)
